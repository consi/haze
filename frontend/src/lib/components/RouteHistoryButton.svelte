...
{#if host.probe_type === 'ping'}
  <button type="button" class="route-button ml-auto shrink-0 inline-flex items-center gap-1 rounded px-1.5 py-0.5 text-[11px]" onclick={()=>open=true} title="Review route changes and packet loss" aria-label={`Route history for ${host.display_name}`}>
...
<style>
  ...
  .route-button:hover,.route-button:focus-visible {color:var(--accent);border-color:var(--accent);outline:none}
</style>
